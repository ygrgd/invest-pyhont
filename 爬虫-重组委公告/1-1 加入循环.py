...
url0 = 'http://www.csrc.gov.cn/pub/newsite/ssgsjgb/bgczwgg/index.htm'
urladd = [url0]
urlstart = 'http://www.csrc.gov.cn/pub/newsite/ssgsjgb/bgczwgg/index_'
for i in range(1, 25):
...
for url in urladd:
    ...
    for each in a:
        ...
        # 根据长度无法得到 公告的类型，还是需要根据文本进行分类
        #l = len(name)
        articles.append([name, add, l])
        i = i + 1
        print(i, name, add)
...
#打开网址传输到file
file = urlopen(
    'http://www.csrc.gov.cn/pub/newsite/ssgsjgb/bgczfkyj/201801/t20180112_332401.html'
)
#从file生成b0
b0 = bs(file, 'lxml')
